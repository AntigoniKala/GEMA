...
protected:
    ...
    c2_function() : sampling_grid(0), no_overwrite_grid(0),
...
public:
    ...
    /// \brief copy constructor
    /// \param src the container to copy
    c2_ptr(const c2_ptr<float_type> &src) : c2_const_ptr<float_type>() { this->set_function(src.get_ptr()); }
    /// \brief get a checked pointer to our owned function
...
template <typename float_type = double>
class c2_const_plugin_function_p
...
public:
    ...
    /// \brief construct the container with a pre-defined function
    c2_const_plugin_function_p(const c2_function<float_type> &f) : c2_plugin_function_p<float_type>() { this->set_function(&f); }
    void set_function(const c2_function<float_type> *f)
    {
...
protected:
    c2_binary_function(float_type (*combiner)(
                           ...
                           float_type *yprime2),
                       const c2_function<float_type> &left,
                       const c2_function<float_type> &right) : c2_function<float_type>(), combine(combiner), Left(left),
    ...
    c2_binary_function(float_type (*combiner)(
        const c2_function<float_type> &left,
        const c2_function<float_type> &right,
        float_type x, float_type *yprime, float_type *yprime2)) : c2_function<float_type>(), combine(combiner),
...
public:
    ...
    c2_scaled_function_p(const c2_function<float_type> &outer,
                         float_type scale) : c2_function<float_type>(), func(outer), yscale(scale) {}
...
public:
    c2_composed_function_p(const c2_function<float_type> &outer,
                           const c2_function<float_type> &inner) : c2_binary_function<float_type>(combine, outer, inner)
    {
...
public:
                              ...
                              float_type *yprime2) /* throw(c2_exception) */
    {
        ...
        if (yprime || yprime2)
        {
            ...
            if (yprime2)
                *yprime2 = (y1 * y1 * ypp0 + y0 * (2 * yp1 * yp1 - y1 * ypp1) - 2 * y1 * yp0 * yp1) / (y1 * y1 * y1);
        }
        else
        {
            y0 = left(x);
...
public:
    ...
    c2_transformation(bool transformed,
                      ...
                      float_type (*xout)(float_type)) : fTransformed(transformed), fHasStaticTransforms(true),
    ...
    c2_transformation(bool transformed) : fTransformed(transformed), fHasStaticTransforms(false),
...
/// \ingroup transforms
template <typename float_type>
class c2_transformation_linear
...
public:
    ...
    c2_function_transformation(
        ...
        const c2_transformation<float_type> &yy) : isIdentity(!(xx.fTransformed || yy.fTransformed)), X(xx), Y(yy) {}
    /// \brief destructor
...
{
public:
    c2_lin_lin_function_transformation() : c2_function_transformation<float_type>(
...
{
public:
    c2_log_log_function_transformation() : c2_function_transformation<float_type>(
...
{
public:
    c2_lin_log_function_transformation() : c2_function_transformation<float_type>(
                                               *new c2_transformation_linear<float_type>,
                                               *new c2_transformation_log<float_type>) {}
...
{
public:
    c2_log_lin_function_transformation() : c2_function_transformation<float_type>(
                                               *new c2_transformation_log<float_type>,
                                               *new c2_transformation_linear<float_type>) {}
...
{
public:
    c2_arrhenius_function_transformation() : c2_function_transformation<float_type>(
...
template <typename float_type = double>
class interpolating_function_p
    : public c2_function<float_type>
{
public:
    ...
    interpolating_function_p<float_type> &load(
        ...
        bool upperSlopeNatural, float_type upperSlope, bool splined = true) /* throw(c2_exception) */;
    ...
    sample_function(const c2_function<float_type> &func,
                    ...
                    bool lowerSlopeNatural, float_type lowerSlope,
                    bool upperSlopeNatural, float_type upperSlope) /* throw(c2_exception) */;
...
///
template <typename float_type = double>
class log_lin_interpolating_function_p
    : public interpolating_function_p<float_type>
{
public:
    /// \brief an empty log-linear cubic-spline interpolating_function_p
    ///
    log_lin_interpolating_function_p() : interpolating_function_p<float_type>(
...
///
template <typename float_type = double>
class lin_log_interpolating_function_p
    : public interpolating_function_p<float_type>
{
public:
...
///
template <typename float_type = double>
class log_log_interpolating_function_p
    : public interpolating_function_p<float_type>
{
public:
    /// \brief an empty log-log cubic-spline interpolating_function_p
    ///
    log_log_interpolating_function_p() : interpolating_function_p<float_type>(
...
///
template <typename float_type = double>
class arrhenius_interpolating_function_p
    : public interpolating_function_p<float_type>
{
public:
...
public:
    ...
    c2_linear_p(float_type x0, float_type y0, float_type slope) : c2_function<float_type>(), xint(x0), intercept(y0), m(slope) {}
...
public:
    ...
    c2_quadratic_p(float_type x0, float_type y0,
                   float_type xcoef, float_type x2coef) : c2_function<float_type>(), intercept(y0), center(x0), a(x2coef), b(xcoef) {}
...
public:
    ...
    c2_power_law_p(float_type scale, float_type power) : c2_function<float_type>(), a(scale), b(power) {}
