...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
    ...
    if (fTallyNumber > 0)
    {
        for (G4int j = 0; j < fTallyNumber; ++j)
        {
            ...
            fTallyMass[j] = fTallySize[j].x() * fTallySize[j].y() * fTallySize[j].z() * (fAbsorMaterial->GetDensity());
        }
    }
    ...
    if (pttoMaterial && pttoMaterial != fAbsorMaterial)
    {
        ...
        for (G4int j = 0; j < fTallyNumber; ++j)
        {
            if (fLTally[j])
            {
                ...
                fTallyMass[j] = fTallySize[j].x() * fTallySize[j].y() * fTallySize[j].z() * (pttoMaterial->GetDensity());
            }
        }
        if (fLAbsor)
        {
