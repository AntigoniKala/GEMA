...
void G4ScreenedCoulombCrossSection::BuildMFPTables()
{
    ...
    for (G4int matidx = 0; matidx < nMaterials; matidx++)
    {
        ...
        for (G4int kel = 0; kel < nMatElements; kel++)
        {
            ...
            if (!kel || ifunc.xmin() > emin)
                emin = ifunc.xmin();
            if (!kel || ifunc.xmax() < emax)
                emax = ifunc.xmax();
        }
...
G4ScreenedNuclearRecoil::
    G4ScreenedNuclearRecoil(const G4String &processName,
                            ...
                            G4double RecoilCutoff, G4double PhysicsCutoff) : G4VDiscreteProcess(processName, fElectromagnetic),
...
G4ScreenedCoulombClassicalKinematics::G4ScreenedCoulombClassicalKinematics() : // instantiate all the needed functions statically, so no allocation is
                                                                               // done at run time
    ...
    const G4ScreeningTables *screen, G4double eps, G4double beta)
{
    ...
    else
    {
        ...
        if (master->CheckNuclearCollision(A, a1, xx0 * au))
            return 0;
        // nuclei too close
    }
                                                   ...
                                                   G4double recoilCutoff)
{
    ...
    for (G4int im = 0; im < nMaterials; im++)
    {
        ...
        for (G4int iEl = 0; iEl < nMatElements; iEl++)
        {
            ...
            if (sigmaMap.find(Z) != sigmaMap.end())
                continue;
            ...
            if (verbosity >= 1)
                G4cout << "Native Screening: " << screeningKey << " "
                       << z1 << " " << a1 << " " << Z << " " << a2 << " " << recoilCutoff << G4endl;
